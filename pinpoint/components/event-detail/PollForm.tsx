import { ActivePoll } from "@/app/(root)/event/[id]";
...
const DateOptionInputs = ({
  addOptions,
  setIsInputShown,
}: {
  addOptions: Dispatch<SetStateAction<Date[]>>;
  ...
  const onDateChange = (event: DateTimePickerEvent, selected?: Date) => {
    if (Platform.OS === "android") return setShowDatePicker(false); // auto-close on Android
  ...
  return (
    <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
      ...
      <View>
        ...
        {msg && <Text className="pt-2 text-red-700">{msg}</Text>}
        <TouchableOpacity onPress={optionSaveHandler} className="py-6">
...
const PlaceOptionInputs = ({
  addOptions,
  setIsInputShown,
}: {
  addOptions: Dispatch<SetStateAction<PlaceOption[]>>;
  setIsInputShown: Dispatch<SetStateAction<boolean>>;
}) => {
...
const OptionLists = <T extends PollQuestion>({
  event_id,
  type,
  title,
  setQuestionDisable,
  ...
  return (
    ...
    >
            ...
            {type === "date" ? (
              ...
              <View>
                ...
                {(option as PlaceOption).url && (
                  ...
                  <Text
                    onPress={() =>
                      Linking.openURL((option as PlaceOption).url!)
                    }
                    style={{ color: "#9ca3af" }}
                  >
                    See More
      ...
      {isInputShown ? (
        ...
        options.length < 3 && (
          <TouchableOpacity onPress={() => setIsInputShown(true)}>
            <View className="flex flex-row items-center gap-3">
              <Entypo name="plus" size={20} color="#D4541A" />
              <Text className="text-[#D4541A] text-[16px] font-LexendMedium">
                Add option
              </Text>
            </View>
          </TouchableOpacity>
      ...
      {options.length > 1 && (
        ...
        >
          <Text className="text-white font-LexendSemiBold text-center text-2xl">
            Create Poll
          </Text>
        </TouchableOpacity>
      )}
...
const PollForm = ({
  ...
  // Active poll existance state
  const [isDateExist, setIsDateExist] = useState<boolean>(false)
  const [isPlaceExist, setIsPlaceExist] = useState<boolean>(false)
  ...
  useEffect(()=>{
    ...
    const datePoll = activePoll.find(p=>p.type==="date")?? null
    const placePoll = activePoll.find(p=>p.type==="place")??null

    if(datePoll) setIsDateExist(true)
    if(placePoll) setIsPlaceExist(true)
        ...
        >
          <Accordion.Header>
            <View className="flex flex-row justify-between">
              <Text className="font-MontserratSemiBold text-2xl">
                Create Poll
              </Text>
          ...
          <Accordion.Expanded>
            ...
            >
              <View className="border-t-[#B0AAA5] py-6">
                {/* poll question */}
                <View className="pb-6">
                  <Text className="text-lg pb-6 font-LexendSemiBold text-[#6B6560]">
                    POLL QUESTION
                  </Text>

                  <View className="border-2 border-[#E2DDD8] rounded-2xl">
                    {!isDateExist&&<TouchableOpacity
                      disabled={questionDisable}
                      onPress={() => setPollQuestion("date")}
                      className={`border-b-2 border-[#E2DDD8] ${pollQuestion === "date" ? "bg-[rgba(9,37,104,0.5)]" : "bg-[#FAFAF9]"} ${isPlaceExist?"rounded-2xl":"rounded-t-2xl"}`}
                    >
                      <Text
                    ...
                    {!isPlaceExist&&<TouchableOpacity
                      disabled={questionDisable}
                      onPress={() => setPollQuestion("place")}
                      className={` ${pollQuestion === "place" ? "bg-[rgba(9,37,104,0.5)]" : "bg-[#FAFAF9]"} ${isDateExist?"rounded-2xl":"rounded-b-2xl"}`}
                    >
                      <Text
